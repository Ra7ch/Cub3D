...
/*   Updated: 2023/11/17 23:28:51 by raitmous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_wall_orientation(t_game *game, double rayangle, int i)
{
	...
	x = (int)(game->wall[i].x / game->xtile);
	y = (int)(game->wall[i].y / game->ytile);
	if (game->map[y][x] == CLOSED_DOOR || ((x - 1 >= 0 && game->map[y][x
				- 1] == CLOSED_DOOR) && game->wall[i].side == HORIZONTAL) || ((y
				- 1 >= 0 && x < game->len[y - 1] && game->map[y
				- 1][x] == CLOSED_DOOR) && game->wall[i].side == VERTICAL))
		game->wall[i].orientation = C_DOOR;
	else
	{
