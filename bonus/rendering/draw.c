...
/*   Updated: 2023/11/17 02:05:04 by mben-zeh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_controls(game);
	if (game->intro_lock || game->shoot)
	{
		mlx_clear_window(game->mlx, game->win);
		raycast(game->pixel_player, game, game->direction);
		(render3dscene(game), draw_background(game), draw_rays(game));
		mlx_put_image_to_window(game->mlx, game->win, game->image->img, 0, 0);
		(mlx_destroy_image(game->mlx, game->image->img), free(game->image));
		game->image = create_image(game->mlx, WINDOW_WEIGHT, WINDOW_HEIGHT);
		game->intro_lock = 0;
	}
	if (game->shoot)
		game->shoot_c++;
	if ((game->shoot_c == 34 * 1 && game->doom == 1) || (game->shoot_c == 69 * 1
			&& game->knif == 1))
	{
		game->shoot_c = 0;
		game->shoot = 0;
	}
	if (game->shoot_c == 110 * 1 && game->knifeattack1 == 1)
	{
		game->shoot = 0;
		game->shoot_c = 69;
	}
